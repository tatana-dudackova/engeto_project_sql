...
-- Dalsi podminky - mzdy a ceny - hranice 10 procent
SELECT DISTINCT 
rok,((GDP/gdp_prev_year*100)-100) AS zmena_hdp,
CASE 
	WHEN ((GDP/gdp_prev_year*100)-100) > 5 THEN 'vyrazny narust hdp'
...
-- Dalsi podminky - mzdy a ceny - hranice 5 procent
SELECT DISTINCT 
rok,((GDP/gdp_prev_year*100)-100) AS zmena_hdp,
CASE 
	WHEN ((GDP/gdp_prev_year*100)-100) > 5 THEN 'vyrazny narust hdp'
...
-- uprava dotazu - odstranuji prvni case, pac je zbytecny
...
-- uprava dotazu: pridavam porovnani pro pristi roky
...
-- tady asi bude dulezity ten komentar. Hranici jsem si u vsech hodnot nakonec zvolila 5
